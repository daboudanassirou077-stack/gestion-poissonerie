...
@section('title', 'État des Stocks - Gérant FreshMarket')
...
<style>
    ...
    .admin-table tfoot td { background:var(--light-bg); font-weight:800; padding:14px 16px; }

    .stock-ok      { background:rgba(39,174,96,.1);   color:var(--green);  padding:4px 12px; border-radius:20px; font-size:11px; font-weight:700; }
    .stock-faible  { background:rgba(232,131,10,.12); color:#a05c00;       padding:4px 12px; border-radius:20px; font-size:11px; font-weight:700; }
    .stock-epuise  { background:rgba(192,57,43,.1);   color:var(--accent); padding:4px 12px; border-radius:20px; font-size:11px; font-weight:700; }

    /* Barre stock */
    .stock-bar { height:8px; border-radius:4px; background:#e0e0e0; overflow:hidden; min-width:80px; }
    ...
    @media print {
        .dash-menu, .dash-header .dash-avatar, nav { display:none !important; }
        ...
        <div class="row">
            ...
            <div class="col-lg-3 mb-4">
                <div class="dash-menu">
                    ...
                    <a href="{{ route('gerant.etat-stock') }}" class="dash-menu-item active">
            ...
            <div class="col-lg-9">
                ...
                @php
                    ...
                    $stock_normal    = $stocks->filter(fn($s) => $s->quantite_stock > $s->seuil_alerte)->count();
                    $stock_faible    = $stocks->filter(fn($s) => $s->quantite_stock > 0 && $s->quantite_stock <= $s->seuil_alerte)->count();
                    $stock_epuise    = $stocks->filter(fn($s) => $s->quantite_stock == 0)->count();
                @endphp
                ...
                {{-- Valeur totale stock --}}
                <div style="background:linear-gradient(135deg,var(--primary),#1a7a9a);border-radius:16px;padding:20px 24px;margin-bottom:24px;display:flex;align-items:center;justify-content:space-between;">
                    <div>
                        <p style="color:rgba(255,255,255,.7);font-size:13px;margin:0 0 4px;">Valeur totale du stock</p>
                        <h2 style="color:#fff;font-size:28px;font-weight:800;margin:0;">
                            {{ number_format($total_valeur, 0, ',', ' ') }} FCFA
                        </h2>
                    </div>
                    ...
                    <table class="admin-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ number_format($stock->produit->prix ?? 0, 0, ',', ' ') }} FCFA</td>
                                <td>
                                    <strong>{{ $stock->quantite_stock }}</strong>
                                    {{ $stock->produit->calibre->unite_vente ?? 'kg' }}
                                </td>
                                <td>
                                    @php
                                        $max = max($stock->seuil_alerte * 3, 1);
                                        $pct = min(100, ($stock->quantite_stock / $max) * 100);
                                ...
                                <td>
                                    <strong>{{ number_format($stock->quantite_stock * ($stock->produit->prix ?? 0), 0, ',', ' ') }} FCFA</strong>
                                </td>
                                <td>
                        ...
                        <tfoot>
                            <tr>
                                <td colspan="6" style="text-align:right;">Total valeur stock :</td>
                                <td colspan="2">{{ number_format($total_valeur, 0, ',', ' ') }} FCFA</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
